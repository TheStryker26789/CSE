...
class R35(Car):
    def __init__(self, name, description, weight):
        super(R35, self).__init__(name, color, description, weight)
        self.name = 'Nissan GT-R'
        self.chassis = 'Premium Midship'
        self.aspiration = turbocharged
        self.hp = 600
        self.torque = 481
        self.style = coupe
...
class Hakosuka(Car):
    def __init__(self, name, color, description, weight):
        super(Hakosuka, self).__init__(name, color, description, weight)
        self.name = 'Nissan Skyline GT-R'
        self.chassis = 'KPGC10'
        self.aspiration = Natural
        self.hp = 170
...
class Eclipse(Car):
    def __init__(self, name, color, description, weight):
        super(Eclipse, self).__init__(name, color, description, weight)
        self.name = 'Mitsubishi Eclipse'
        self.chassis = 'PJ'
        self.aspiration = turbocharged
        self.hp = 210
        self.torque = 180
...
class Viper(Car):
    def __init__(self, name, color, description, weight):
        super(Viper, self).__init__(name, color, description, weight)
...
class Hellcat(Car):
    def __init__(self, name, color, description, weight):
        super(Hellcat, self).__init__(name, color, description, weight)
        self.name = 'Dodge Challenger SRT Hellcat'
        self.chassis = 'LC'
        self.aspiration = supercharged
        self.hp = 707
        self.torque = 650
...
class Demon(Car):
    def __init__(self, name, color, description, weight):
        super(Demon, self).__init__(name, color, description, weight)
        self.name = 'Dodge Challenger SRT Demon'
        self.chassis = 'LC'
        self.aspiration = supercharged
        self.hp = 840
        self.torque = 756
...
class GT350R(Car):
    def __init__(self, name, color, description, weight):
        super(GT350R, self).__init__(name, color, description, weight)
        self.name = 'Ford Mustang GT350R'
        self.chassis = None
        self.aspiration = Natural
        self.hp = 540
        self.torque = 430
...
class Player:  # The Player class stores all attributes of the player character, and allows
    # the player to interact with the game

    def __init__(self, name, health, bag, place):
        self.name = name
        self.health = health
        self.bag = bag
        self.place = West_House

        def travel(self, direction):  # The travel function takes 2 args, the 'direction' arg
            # is provided by player command
            if direction not in self.room.exits.keys():  # Checks if the direction arg does
                # not correspond to an exit of the current room
                print("You can't go that way!")  # If direction does not match up to a
                # possible exit, this string is displayed to the player
            else:  # If direction DOES match up to a possible exit
                new_room_name = self.room.exits[direction]  # Direction arg is used as key to
                # self.room.exits dict; Variable named new_room_name is created and set to value that corresponds to key
                print("moving to", West_House)  # Prints "moving to" whatever room you are moving to
            if world[West_House].lock is True and world[West_House].roomkey not in self.bag:
                # Checks to see if the room you are attempting to move to's lock status is
                # set to true, then checks to see if player inventory does not contain the key that opens that lock
                print('The door is locked! You need the right key!')  # if player inventory
                #  does NOT contain the key, this message is displayed to the player
            else:  # Otherwise
                world[West_House].lock = False  # change the room you are attempting to move to's lock status to False
                self.room = world[West_House]  # and set the current room equal
                # to the room that you are attempting to move to


player = Player("Fujiwara", 100, [West_House], 'introd')
